...
- [x] 管理员可查看用户列表，并修改其他用户角色和状态。
- [x] 管理员可直接设置积分，也可按 delta 增减积分，并写流水。
- [x] 管理员可修改注册、审核、注册送积分、生成扣费、签到奖励等系统设置。
- [x] 初始系统设置默认开放注册，默认不需要审核。
- [x] 每次生成请求都会写审计记录，包含 IP、User-Agent、状态和错误摘要。
- [x] 审计页可按最新时间查看生成请求和关联输出。
- [x] 后台接口不返回原始密钥、token、cookie 或数据库密码。
- [x] 通过 `pnpm typecheck`。
- [x] 通过 `pnpm build`。
- [x] 运行 `pnpm dev` 后，用内置浏览器验证后台桌面/移动基本可用。
