...
function generationWarningMessage(record: GenerationRecord, insertedCount: number, failedCount: number, t: Translate): string {
...
export function App({ currentUser, sessionError = "", onLogout }: CanvasAppProps) {
  ...
  function finishPolledGeneration(record: GenerationRecord, placeholderSet: ActiveGenerationPlaceholders, notify: boolean): void {
    ...
    if (insertedCount > 0) {
      if (failedCount > 0) {
        setGenerationWarning(generationWarningMessage(record, insertedCount, failedCount, t));
      } else {
