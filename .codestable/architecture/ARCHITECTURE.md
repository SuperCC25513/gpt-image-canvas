...
- **生成 Provider 调度规划**：`.codestable/roadmap/generation-provider-scheduler/` 记录 Redis runtime、全局 provider 并发闸门、队列、重试、状态桥和 Agent 接入的拆分；当前已完成 Redis runtime 底座和全局 provider 并发闸门。
...
- 当前已实现 provider permit 并发闸门，但还没有 generation queue worker、provider retry policy、取消恢复或队列可观测性；这些仍由 `generation-provider-scheduler` 后续子 feature 实现。
